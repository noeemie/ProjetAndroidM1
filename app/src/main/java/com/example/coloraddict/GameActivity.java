    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for (int j = 0; j < 2; j++) {
            ...
            for (i = 0; i < session.getPlayer(j).getHandSize(); i++) {
                    ...
                    public void onClick(View v) {// action de jouer la carte
                        ...
                        if(cardPlay2.isOK(session.getStack())){
                            ...
                            if(session.getPlayer(playerInt).getHandSize() < 3){
                                ...
                                session.getPlayer(playerInt).getRandomCardStack();
                                Button newCardPlay1 = new Button(v.getContext());
                                newCardPlay1.setText(session.getPlayer(playerInt).getLastCardHand().getNameColor().toString()); // aller chercher la carte dans la liste du joueur en train de jouer
                                newCardPlay1.setTextColor(session.getPlayer(playerInt).getCardHand().getColor()); // couleur du texte de la carte du joueur

                                newCardPlay1.setOnClickListener(new View.OnClickListener() {
                                    @Override
                                    public void onClick(View v) {// action de jouer la carte
                                        //prendre la carte de la liste
                                        Card cardPlay2 = session.getPlayer(playerInt).getCardHand();
                                        // tester si la carte est bonne
                                        if(cardPlay2.isOK(session.getStack())){
                                            // la partie ne s'arrete que quand les n-1 joueurs ont le statut WIN
                                            if(session.isFinished()){
                                            ...
                                            if(session.getPlayer(playerInt).isEmptyHand()){
                                                session.getPlayer(playerInt).win();
                                            }
                ...
                card.setTextColor(session.getPlayer(playerInt).getCardHand().getColor()); // couleur du texte de la carte du joueur
        ...
        mDrawACard.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                newCard.setText(session.getPlayer(playerInt).getLastCardHand().getNameColor().toString()); // aller chercher la carte dans la liste du joueur en train de jouer
                newCard.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {// action de jouer la carte
                        //prendre la carte de la liste
                        Card cardPlay1 = session.getPlayer(playerInt).getCardHand();
                        // tester si la carte est bonne
                        if(cardPlay1.isOK(session.getStack())){
                            // la partie ne s'arrete que quand les n-1 joueurs ont le statut WIN
                            if(session.isFinished()){
                                // aller a l'écran des scores
                            }
                            ...
                            newCardPlay2.setText(session.getPlayer(playerInt).getLastCardHand().getNameColor().toString()); // aller chercher la carte dans la liste du joueur en train de jouer
                            newCardPlay2.setTextColor(session.getPlayer(playerInt).getCardHand().getColor()); // couleur du texte de la carte du joueur
                ...
                newCard.setTextColor(session.getPlayer(playerInt).getCardHand().getColor()); // couleur du texte de la carte du joueur
                ...
                if(session.getPlayer(playerInt).isEmptyStack()){
